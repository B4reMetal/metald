...
examples/chatbot.yml   canonical config; carries the default text of every prompt
examples/env.example   every env var the tools read, with placeholders
...
**Prompts have no built-in defaults.** The six keys in `config.PromptKeys` (`floorprompt`, `gatekeeperpreamble`, `gatekeeperpolicy`, `classifypreamble`, `replyscreenpolicy`, `memorypolicy`) must be set in config; `NewConfiguration` exits 1 with `prompts_missing keys=...` otherwise. Never add a `Value:` default or a Go const for prompt text. Default text lives only in `examples/chatbot.yml`, guarded by `TestShippedExampleConfigHasEveryPrompt`. Tests that need real prompt text load it with `shippedPrompt(t, key)`. Same rule for Python tools: `MUSIC_SAFETY_POLICY` and `CAT_PIC_ROAST_PROMPT` are required env vars with defaults only in `examples/env.example`.
...
- No secrets, LAN addresses, real hostnames or real user nicks anywhere in the tree, including test fixtures and comments. Use `example.com`, `127.0.0.1`, and the fixture nicks already in use (`alice`, `bob`, `mallory`, `eve`).
